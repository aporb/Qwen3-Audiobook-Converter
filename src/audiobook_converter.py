...
def get_device_and_dtype():
    """Auto-detect best device and dtype for model inference.

    Returns:
        tuple: (device_string, torch_dtype)
        - CUDA: Uses bfloat16 for better performance
        - MPS (Apple Silicon): Uses float32 (voice clone requires it)
        - CPU: Uses float32 as fallback
    """
    if torch.cuda.is_available():
        return "cuda", torch.bfloat16
    elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
        return "mps", torch.float32  # Voice clone requires float32 on MPS
    else:
...
class QwenAudiobookConverter:
    ...
    def init_model(self):
        """Initialize Qwen3-TTS model directly with auto device detection"""
        try:
            ...
            print(f"[INFO] Detected device: {self.device}")
            ...
            print(f"[INFO] Loading Qwen3-TTS model ({MODEL_ID})...")
            print("[INFO] This may take a few minutes on first run (downloading ~6GB model)...")
            self.model = Qwen3TTSModel.from_pretrained(
                MODEL_ID,
                device_map=self.device,
                torch_dtype=self.dtype
            )
            self.logger.info("Qwen3-TTS model loaded successfully")
            ...
            print("Make sure:")
            print("1. You have enough memory (16GB+ recommended)")
            print("2. PyTorch is installed with proper backend support")
            print("3. The qwen-tts package is installed correctly")
            print(f"Error: {e}")
            import traceback
            traceback.print_exc()
            sys.exit(1)
    ...
    def generate_chunk_via_qwen(self, text: str, chunk_num: int) -> Optional[str]:
        """Generate audio chunk using Qwen model"""
        try:
            # Check cache first
            ...
            if self.voice_mode == "custom_voice":
                audio_array, sample_rate = self._generate_custom_voice(text)
            elif self.voice_mode == "voice_clone":
                audio_array, sample_rate = self._generate_voice_clone(text)
            else:
    ...
    def _generate_custom_voice(self, text: str) -> Tuple[np.ndarray, int]:
    ...
    def _generate_voice_clone(self, text: str) -> Tuple[np.ndarray, int]:
        ...
        wavs, sr = self.model.generate_voice_clone(
            ...
            x_vector_only_mode=VOICE_CLONE_USE_XVECTOR_ONLY,
            max_new_tokens=2048,
            non_streaming_mode=True
        )
        return wavs[0], sr
