#!/usr/bin/env python3
"""
Qwen3-TTS Audiobook Converter - VoiceDesign Pipeline
=====================================================
Converts EPUB/PDF/TXT to audiobooks using AI-designed voices.

Unlike voice cloning, VoiceDesign lets you describe the voice you want
(e.g., "American male, professional narrator, baritone voice").
...
Usage:
    python convert_audiobook_voicedesign.py --epub book.epub --config voicedesign_american_male.yaml
    python convert_audiobook_voicedesign.py --epub book.epub --config voicedesign_american_male.yaml --dry-run
"""
...
DEFAULT_CONFIG = {
    ...
    'voice': {
        'slug': 'voicedesign',
        'instruct': 'Adult American male, baritone voice, professional audiobook narrator, clear enunciation, calm and authoritative tone.',
        'language': 'English'
    },
    'chapters': {
        'include': None,
...
# ============================================================
# VOICEDESIGN AUDIOBOOK CONVERTER
# ============================================================

class VoiceDesignConverter:
    """Convert books to audiobooks using VoiceDesign (AI-designed voices)."""
    ...
    def __init__(
        ...
        # Voice configuration
        self.voice_instruct = self.config['voice'].get('instruct', '')
        self.voice_language = self.config['voice'].get('language', 'English')
        self.voice_slug = self.config['voice'].get('slug', 'voicedesign')
    ...
    def load_models(self):
        """Load VoiceDesign TTS model."""
        print(f"[*] Device: {self.device} ({self.dtype})")
        print(f"[*] Available memory: {get_available_memory_gb():.1f} GB")

        print(f"[*] Loading VoiceDesign model (~6GB)...")
        print(f"    Model: {VOICEDESIGN_MODEL_ID}")
        self.tts_model = Qwen3TTSModel.from_pretrained(
            VOICEDESIGN_MODEL_ID,
        ...
        print(f"\n[*] Voice Configuration:")
        print(f"    Language: {self.voice_language}")
        print(f"    Description: {self.voice_instruct[:80]}...")
        print(f"    Slug: {self.voice_slug}")

        print(f"\n[*] Available memory after loading: {get_available_memory_gb():.1f} GB")
    ...
    def convert_epub(
        ...
        # Output naming with voice slug
        if output_path:
            output_path = Path(output_path)
        else:
            fmt = self.config['output'].get('format', 'm4b')
            output_path = self.output_dir / f"{book_name}_{self.voice_slug}.{fmt}"

        state_file = Path(state_path) if state_path else self.output_dir / f".{book_name}_{self.voice_slug}.progress.json"
        ...
        if state_file.exists():
            try:
                ...
                print(f"[*] Resuming from checkpoint ({len(state.chapters_completed)} items done)")
            except Exception:
                state = None
    ...
    def dry_run(self, epub_path: str):
        ...
        print(f"\n[DRY RUN] VoiceDesign Pipeline")
        print(f"[DRY RUN] Parsing EPUB: {epub_path}")
        ...
        print(f"Slug:     {self.voice_slug}")
        print(f"Language: {self.voice_language}")
        print(f"Description: {self.voice_instruct}")
        ...
        output_name = f"{book_name}_{self.voice_slug}.{fmt}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert EPUB/PDF to audiobook using VoiceDesign (AI-designed voices)",
...
Examples:
  ...
  python convert_audiobook_voicedesign.py --epub book.epub --config voicedesign_american_male.yaml --resume
        """
    )

    parser.add_argument('--epub', '-e', type=str, help='Path to EPUB file')
    parser.add_argument('--config', '-c', type=str, required=True, help='Path to YAML config file (required)')
    ...
    if not config:
        print("[!] Config is required for VoiceDesign pipeline")
        sys.exit(1)

    # Create converter
    converter = VoiceDesignConverter(
        config=config,
